...
export function validateManualSubscription(input: unknown) {
  ...
  return { serviceName, billingFrequency: value.billingFrequency as string, renewalOrEndDate: parsed, amount: amount || undefined, requestId };
}
...
export function createSubscriptionStore(client: DbClient = prisma) {
  return {
    async list(user: JWTPayload) {
      const records = await client.subscription.findMany({ where: { userEmail: user.email }, orderBy: [{ renewalOrEndDate: 'asc' }, { createdAt: 'asc' }] });
